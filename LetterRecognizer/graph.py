...
from .object import weight_variable, bias_variable, max_pool_2x2, conv2d
...
def placeholder_inputs(batch_size):
    ...
    w_fc1 = weight_variable([12*7*64, 12*7*64], "w_fc1")
    w_fc2 = weight_variable([12*7*64, 11], "w_fc2")
    W = Weight(w_conv1,w_conv2,w_fc1,w_fc2)

    b_conv1 = bias_variable([32], "b_conv1")
    b_conv2 = bias_variable([64], "b_conv2")
    b_fc1 = bias_variable([12*7*64], "b_fc1")
    b_fc2 = bias_variable([11], "b_fc2")
    B = Weight(b_conv1, b_conv2, b_fc1, b_fc2)

    keep_prob = tf.placeholder(tf.float32)

    x = tf.placeholder(tf.float32, shape=[batch_size, HYPARMS.input_shape[0],HYPARMS.input_shape[1]])
...
def graph_model(placebundle):
    ...
    # 48*28*1 -> 12*7*64
    x_image = tf.reshape(x, [-1, HYPARMS.input_shape[0], HYPARMS.input_shape[1], 1])
    h_conv1 = tf.nn.relu(conv2d(x_image, W_conv1) + b_conv1)
    h_pool1 = max_pool_2x2(h_conv1)
    h_conv2 = tf.nn.relu(conv2d(h_pool1, W_conv2) + b_conv2)
    h_pool2 = max_pool_2x2(h_conv2)

    h_pool2_flat = tf.reshape(h_pool2, [-1, 12*7*64])
